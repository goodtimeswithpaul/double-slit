...
def main():
	...
	# === Run main function ===
    start = time.time()
    cythonfn.simulate_finite_difference(U, Uprev, mask, float(boxsize), int(N), float(c), cmap, float(tEnd), plotRealTime)
    ...
    start = time.time()
    simulate_finite_difference(U, Uprev, mask, float(boxsize), int(N), float(c), cmap, float(tEnd), plotRealTime)
    print("cython: ", time.time() - start)
